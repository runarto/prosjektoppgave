#!/usr/bin/env python3
"""
Magnetometer Bias Test WITHOUT Star Tracker.

Tests whether removing normalization makes the M-estimator more effective
when the magnetometer is a primary sensor (no star tracker).
"""
...
def run_estimators(sim_data, config_path, fault_config, normalize_mag=True, use_star_tracker=True):
    """Run ESKF and Smoother with specified settings."""
    ...
    for k in range(1, len(sim_data.t)):
        ...
        # Optionally disable star tracker
        z_st = None
        if use_star_tracker and not np.any(np.isnan(sim_data.st_meas[k])):
...
def main():
    ...
    print("Star tracker: DISABLED\n")

    print("Running with normalization...")
    res_norm = run_estimators(sim_data, config_path, fault_config, normalize_mag=True, use_star_tracker=False)

    print("Running WITHOUT normalization...")
    res_raw = run_estimators(sim_data, config_path, fault_config, normalize_mag=False, use_star_tracker=False)

    # Create comparison plot
    fig, axes = plt.subplots(2, 2, figsize=(12, 8))
    fig.suptitle(f'Magnetometer Bias (No Star Tracker): Normalized vs Raw (|bias|={bias_mag:.2f})', fontsize=12)

    t_start = 60.0
    fault_onset, fault_end = fault_config['onset_time'], fault_config['end_time']

    scenarios = [
        (axes[0], res_norm, 'With Normalization'),
    ...
    plt.tight_layout()
    plt.savefig('mag_bias_no_st_test.pdf', dpi=150, bbox_inches='tight')
    plt.savefig('mag_bias_no_st_test.png', dpi=150, bbox_inches='tight')
    print("\nSaved to mag_bias_no_st_test.pdf/png")
    plt.close()

    # Print comparison
    print("\n" + "="*70)
    print("COMPARISON SUMMARY (No Star Tracker)")
    print("="*70)
