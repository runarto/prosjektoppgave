...
def run_eskf_and_save(sim_run_id: int, db_path: str = "simulations.db",
    ...
    # Initialize ESKF
    att_err = np.pi
    P0 = np.diag([att_err**2, att_err**2, att_err**2, 1e-4, 1e-4, 1e-4])
    ...
    q_err = Quaternion.from_avec(np.array([att_err, att_err, att_err]))
    q0_est = q0_true @ q_err
    b0_est = np.array([0.0, 0.0, 0.0])  # initial gyro bias estimate
    ...
    q_err = (q_true_final.conjugate() @ q_est_final).normalize()
...
def main():
    ...
    # Configuration
    sim_run_id = 18  # Use run with STIM300 parameters
